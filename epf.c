...
char another = 'y';
int id; 
float final_salary, basic, net_salary;
float pf = 0;
int emplyCntrbtn, emplyrCntrbtn;
printf("Enter Employee id:");
scanf("%d", &id);
printf("Enter Basic Salary ($): ");
scanf("%f", &basic);
   // Calculate Employee Contribution
    emplyCntrbtn = (basic * 12) / 100;
    // calculate Employer Contribution
    emplyrCntrbtn = (basic * 13) / 100;
    pf = emplyCntrbtn + emplyrCntrbtn;
    printf("PF Is %f", pf);
// calculate net salary
net_salary = basic + pf;
printf("\nNet Salary is: %f\n", net_salary);
final_salary = (12*pf)+8/100;
 printf("\nfinal_Salary is: %f\n", final_salary);
printf("\nDo you want to calculate another record (Y/N) :");
 fflush(stdin);
    another = getch();
}
